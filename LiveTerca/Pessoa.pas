...
type
  TPessoa = class
  type
    TBuilder = class { classe aninhada, que na programação e uma subrotina
      encapsulada em outra, isso quer dizer que a classes aninhadas são classes
      que estao dentro de outra classe, que ingles quer dizer Nested Classes,
      * Agrupamento de classes que são usadas em apenas um lugar
      * Aumento de encapsulamento
      * Código mais legivel e de facil manutenção }
    private
      FNome : String;
      FSobreNome : String;
      FNascimento : String;
...
begin
  Result := Self;
  FTelefone.Add(TTelefone.Create(aDDD,aNumero,
  TTipoTelefone(GetEnumValue(TypeInfo(TTipoTelefone), Tipo))));
end;
